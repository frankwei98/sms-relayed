	...
	config: {
		...
		status: {
			...
			restartCommandFailed:
				"サービスの再起動コマンドに失敗しました。現在のセッションは有効なままです。再起動を再試行してください。",
			restartStatusUnavailable:
				"サービスの再起動を確認できません。現在のセッションは有効なままです。再起動を再試行してください。",
		},
		restartDialog: {
		...
		warnings: {
			passwordChange:
				"サービスが再起動するまで現在のセッションは有効です。新しいサービスの起動後にすべてのセッションが無効になります。",
		...
		fields: {
			...
			api: {
				...
				password: "パスワード",
				passwordDescription:
					"この値を変更すると、保存と再起動のスケジュールが一度に行われます。新しいサービスの起動後にのみ、すべてのセッションが無効になります。",
